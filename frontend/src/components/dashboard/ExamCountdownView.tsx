...
// calculates the time remaining until an exam date
...
// returns colour scheme based on urgency
...
// exam countdown view — shows all upcoming exams with live countdown timers.
// colour coded by urgency: green (14+ days), amber (3-14 days), red (under 3 days).
function ExamCountdownView() {
  const { exams, isLoading, fetchExams, createExam, deleteExam } = useExamStore()
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      await createExam(name, subject || null, examDate)
      setName('')
      setSubject('')
      setExamDate('')
      setShowForm(false)
  ...
  const upcomingExams = exams.filter((e) => getTimeRemaining(e.exam_date) !== null)
  const pastExams = exams.filter((e) => getTimeRemaining(e.exam_date) === null)
  ...
  return (
    <div>
      ...
      {/* upcoming exams */}
      {isLoading ? (
        <div style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>Loading exams...</div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)' }}>
            Add your exams to start tracking countdowns.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div style={{
                  ...
                  color: urgency.color, fontSize: '0.75rem', fontWeight: 600,
                  flexShrink: 0,
                }}>
